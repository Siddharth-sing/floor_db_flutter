    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `Bookmark` (`id` INTEGER, `catId` INTEGER, `pageNo` INTEGER, PRIMARY KEY (`id`))');
...
class _$BookmarkDao extends BookmarkDao {
  ...
  Future<List<Bookmark>> findAllBookmark() async {
    return _queryAdapter.queryList('SELECT * FROM Bookmark',
        mapper: (Map<String, dynamic> row) => Bookmark(
            row['id'] as int, row['catId'] as int, row['pageNo'] as int));
  }

  @override
  Future<void> insertBookmark(Bookmark bookmark) async {
    await _bookmarkInsertionAdapter.insert(bookmark, OnConflictStrategy.abort);
  }
